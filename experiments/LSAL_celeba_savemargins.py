...
from blocks.extensions import FinishAfter, Timing, Printing, ProgressBar,Timestamp
...
from ali.bricks_savemargin import (ALI, GaussianConditional, DeterministicConditional,
...
def create_model_brick():
    ...
    ali.initialize()
    #tmp=create_celeba_data_streams(500, 500)[0].get_epoch_iterator()
    #print('tmp :',tmp.shape)
    raw_marginals, = next(
...
def create_main_loop(save_path,file_name):
    model, bn_model, bn_updates = create_models()
    ali, = bn_model.top_bricks
    #discriminator_loss, generator_loss,delta_z,delta_x = bn_model.outputs
    ...
    monitored_variables = (
        ...
        #model.outputs)
    time=Timestamp()
    #cp=Checkpoint(save_path+str(datetime.now())+file_name, after_batch=True, after_epoch=True, after_training=True,
     #              use_cpickle=True)
    #print(cp[0])
    extensions = [
        Timing(),
        ...
        Checkpoint(save_path+str(datetime.now())+file_name, after_epoch=True, after_training=True,
                   use_cpickle=True),

       #cp.do(save_path+str(datetime.now())+file_name),
        ProgressBar(),
        Printing(),
    ]
    counter=counter+1
    ...
    #cp=main_loop.find_extension('Checkpoint')
    #cp.do(save_path+str(datetime.now())+file_name)
    return main_loop
...
if __name__ == "__main__":
    ...
    parser.add_argument("--save-path", type=str, default='/home/newmoon/GAN/ALI/celebA_margins_model/',
                        help="main loop save path")
    parser.add_argument("--file_name", type=str, default='LSAL_celeba_margins.tar',
                        help="main loop save path")
    args = parser.parse_args()
    create_main_loop(args.save_path,args.file_name).run()
