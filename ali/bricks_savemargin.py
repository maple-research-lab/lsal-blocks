...
class ALI(Initializable, Random):
    ...
    @application(inputs=['x', 'z_hat', 'x_tilde', 'z'],
                 outputs=['data_preds', 'sample_preds','delta_z','delta_x'])
    def get_predictions(self, x, z_hat, x_tilde, z, application_call):
        ...
        delta_x=tensor.abs_((x-x_tilde))
        delta_x=tensor.sum(delta_x,axis=(1,2,3),keepdims=True)



            #((x-x_tilde).abs_()).sum(axis=(1,2,3))#tensor.norm((x-x_tilde),(1,2,3))#tensor.sum(tensor.)
        delta_z=tensor.abs_((z_hat-z))
        delta_z=tensor.sum(delta_z,1,keepdims=True)

            #((z_hat-z).abs_()).sum(axis=1)#tensor.norm((z_hat-z),1)
        return data_preds, sample_preds,delta_z,delta_x

    @application(inputs=['x', 'z'],
                 outputs=['discriminator_loss', 'generator_loss'])#,'delta_z','delta_x'])
    def compute_losses(self, x, z, application_call):
        ...
        data_preds, sample_preds,delta_z,delta_x = self.get_predictions(x, z_hat, x_tilde, z)

        discriminator_loss=tensor.nnet.relu(self.alpha*delta_z+self.beta*delta_x+data_preds-sample_preds)#(delta_z+data_preds-sample_preds) #self.alpha*delta_z+self.beta*delta_x+
         #   Rectifier(self.alpha*delta_z+self.beta*delta_x+data_preds-sample_preds)
        application_call.add_auxiliary_variable(
            self.alpha*delta_z.mean(),
            name='delta_z')
        application_call.add_auxiliary_variable(
            self.beta*delta_x.mean(), name='delta_x')

        discriminator_loss=discriminator_loss.mean()
        generator_loss=sample_preds.mean()-data_preds.mean()
       ...
       # discriminator_loss = (tensor.nnet.softplus(-data_preds) +
        ...
        return discriminator_loss, generator_loss#,delta_z,delta_x
