...
def create_svhn_data_streams_cls(batch_size, monitoring_batch_size, rng=None):
    train_set = SVHN(3, ('extra',), sources=('features','targets','one_hot'))
    valid_set = SVHN(3, ('train',), sources=('features','targets','one_hot'))
    test_set =  SVHN(3, ('test',), sources=('features','targets','one_hot'))
    main_loop_stream = DataStream.default_stream(
        train_set,
        iteration_scheme=ShuffledScheme(
            train_set.num_examples, batch_size, rng=rng))
    ...
    valid_monitor_stream = DataStream.default_stream(
        ...
        iteration_scheme=ShuffledScheme(
            5000, monitoring_batch_size, rng=rng))
    test_monitor_stream = DataStream.default_stream(test_set,iteration_scheme=SequentialScheme(test_set.num_examples,batch_size))
    return main_loop_stream, train_monitor_stream, valid_monitor_stream,test_monitor_stream
